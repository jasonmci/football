...
class PlayerRole:
    ...
    alignment: Optional[str] = (
        None  # Specific alignment within depth (tight, slot, etc.)
    )
...
class Formation(ABC):
    ...
    def get_roles_by_position(self, position: Position) -> Dict[str, PlayerRole]:
        ...
        return {
            name: role for name, role in self.roles.items() if role.position == position
        }
...
class PositionConstraints:
    ...
    def add_distance_constraint(
        self, position1: str, position2: str, min_distance: int, max_distance: int
    ) -> None:
    ...
    def add_depth_constraint(
        self, position: str, min_depth: int, max_depth: int
    ) -> None:
    ...
    def validate_formation(self, formation: Formation) -> List[str]:
        ...
        for (pos1, pos2), (min_dist, max_dist) in self.distance_constraints.items():
            violations.extend(
                self._check_distance_constraint(
                    formation, pos1, pos2, min_dist, max_dist
                )
            )
    ...
    def _check_distance_constraint(
        self, formation: Formation, pos1: str, pos2: str, min_dist: int, max_dist: int
    ) -> List[str]:
    ...
    def _check_depth_constraint(
        self, formation: Formation, position: str, min_depth: int, max_depth: int
    ) -> List[str]:
