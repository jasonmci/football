...
from .plays import FootballPlay, AssignmentType
...
class PlayAnalyzer:
    ...
    def analyze_play_matchup(
        self, offensive_play: FootballPlay, defensive_play: FootballPlay
    ) -> PlayAnalysis:
        """
        ...
        blocking_analysis = self._analyze_blocking_scheme(
        ...
        coverage_analysis = self._analyze_coverage_scheme(
            offensive_play, defensive_play
        )
        ...
        motion_analysis = self._analyze_motion_advantages(
            offensive_play, defensive_play
        )
    ...
    def _analyze_blocking_scheme(
        self, offense: FootballPlay, defense: FootballPlay
    ) -> Dict[str, Any]:
        ...
        if blockers > rushers:
            ...
            impact = min(
                1, blockers - rushers
            )  # Reduced from min(2, blockers - rushers)
            advantages.append(
                PlayMatchupFactor(
            ...
            impact = max(
                -2, -(rushers - blockers)
            )  # Reduced from max(-3, -(rushers - blockers))
    ...
    def _analyze_motion_advantages(
        self, offense: FootballPlay, defense: FootballPlay
    ) -> Dict[str, Any]:
    ...
    def _count_rushers(self, defense: FootballPlay) -> int:
        ...
        return sum(
            ...
            or assign.assignment_type == AssignmentType.BLITZ
        )
    ...
    def _count_blockers(self, offense: FootballPlay) -> int:
        ...
        return sum(
            ...
            in [AssignmentType.RUN_BLOCK, AssignmentType.PASS_BLOCK]
        )
    ...
    def _find_crack_blocks(self, offense: FootballPlay) -> List[str]:
        ...
        for assign in offense.assignments:
            if assign.assignment_type == AssignmentType.RUN_BLOCK:
                ...
                if details.get(
                    "technique"
                ) == "crack" and assign.player_position.startswith("WR"):
    ...
    def _find_trap_blocks(self, offense: FootballPlay) -> List[str]:
        ...
        for assign in offense.assignments:
            if assign.assignment_type == AssignmentType.RUN_BLOCK:
                ...
                if (
                    details.get("scheme") == "pull"
                    and details.get("technique") == "trap_block"
                ) or (
                    details.get("scheme") == "trap_set"
                    and details.get("technique") == "invite_penetration"
                ):
    ...
    def _has_basic_protection(self, offense: FootballPlay) -> bool:
        ...
        basic_count = sum(
            ...
            if scheme in ["basic", "big_on_big", "vertical_set"]
        )
        return basic_count > len(protection_schemes) / 2

    def _find_coverage_mismatches(
        self, offense: FootballPlay, defense: FootballPlay
    ) -> List[PlayMatchupFactor]:
        ...
        rb_routes = [
            assign
            for assign in offense.assignments
            if assign.player_position.startswith("RB")
            and assign.assignment_type == AssignmentType.ROUTE
        ]
    ...
    def _get_run_direction(self, offense: FootballPlay) -> Optional[str]:
        ...
        for assign in offense.assignments:
            if (
                assign.assignment_type == AssignmentType.RUN_BLOCK
                and assign.player_position.startswith("RB")
            ):
    ...
    def _is_power_concept(self, offense: FootballPlay) -> bool:
        ...
        has_lead_blocker = any(
            assign.assignment_type == AssignmentType.LEAD_BLOCK
            for assign in offense.assignments
        )
        ...
        for assign in offense.assignments:
            if assign.assignment_type == AssignmentType.HANDOFF:
                ...
                if isinstance(details, str):
                    details = {}
                if (
                    details.get("fake_direction")
                    or details.get("technique") == "counter_step"
                ):
                    return True
        return False

    def _defense_overloaded_opposite(
        self, defense: FootballPlay, run_direction: str
    ) -> bool:
    ...
    def _init_blocking_schemes(self) -> Dict[str, Any]:
        ...
        return {
            "zone": {
                ...
                "weaknesses": ["power", "stunts"],
            },
            "power": {
                "strengths": ["gaps", "physical"],
                "weaknesses": ["speed", "edges"],
            },
    ...
    def _init_coverage_schemes(self) -> Dict[str, Any]:
        ...
        return {
            "man": {
                "strengths": ["individual_coverage"],
                "weaknesses": ["picks", "crossing"],
            },
            "zone": {
                "strengths": ["help", "robber"],
                "weaknesses": ["seams", "levels"],
            },
        }
