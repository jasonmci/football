...
class FormationMatchupAnalyzer:
    ...
    def _initialize_offensive_strengths(self) -> Dict[str, FormationStrengths]:
        ...
        return {
            "empty_backfield": FormationStrengths(
                ...
                route_diversity=5,
                misdirection=3,
                optimal_play_types=[PlayType.PASS_SHORT, PlayType.PASS_DEEP, PlayType.SCREEN]
            ),
            "spread_10": FormationStrengths(
                ...
                route_diversity=4,
                misdirection=4,
                optimal_play_types=[PlayType.RUN_OUTSIDE, PlayType.PASS_SHORT, PlayType.SCREEN]
            ),
            ...
            "pistol_11": FormationStrengths(
                ...
                route_diversity=3,
                misdirection=4,
                optimal_play_types=[PlayType.RUN_INSIDE, PlayType.RUN_OUTSIDE, PlayType.PLAY_ACTION]
            ),
            ...
            "singleback_11": FormationStrengths(
                ...
                route_diversity=3,
                misdirection=3,
                optimal_play_types=[PlayType.RUN_INSIDE, PlayType.PASS_SHORT, PlayType.PLAY_ACTION]
            )
        }
    ...
    def _initialize_defensive_strengths(self) -> Dict[str, DefenseStrengths]:
        ...
        return {
            ...
            "base43": DefenseStrengths(
                ...
                pass_coverage=3,
                gap_control=4,
                counters_play_types=[PlayType.RUN_INSIDE, PlayType.RUN_OUTSIDE, PlayType.PASS_SHORT]
            ),
    ...
    def analyze_matchup(self, offense_name: str, defense_name: str) -> MatchupResult:
        ...
        pass_differential = (offense.pass_protection + offense.route_diversity) / 2 - (defense.pass_rush + defense.pass_coverage) / 2
        ...
        key_factors = self._identify_key_factors(offense, defense, run_differential, pass_differential)
        
        # Recommend plays based on advantages
        recommended_plays = self._recommend_plays(offense, defense, run_advantage, pass_advantage)
    ...
    def _identify_key_factors(self, offense: FormationStrengths, defense: DefenseStrengths, 
                            run_diff: float, pass_diff: float) -> List[str]:
        """Identify the key strategic factors in this matchup."""
        factors = []
        
        if run_diff >= 1.0:
            factors.append(f"Offense has strong run blocking advantage ({offense.run_blocking} vs {defense.run_defense})")
        elif run_diff <= -1.0:
            factors.append(f"Defense dominates run game ({defense.run_defense} vs {offense.run_blocking})")
        
        if pass_diff >= 1.0:
            factors.append(f"Offense has passing game advantage")
        elif pass_diff <= -1.0:
            factors.append(f"Defense controls passing game")
    ...
    def _recommend_plays(self, offense: FormationStrengths, defense: DefenseStrengths,
                        run_advantage: MatchupAdvantage, pass_advantage: MatchupAdvantage) -> List[PlayType]:
        """Recommend optimal play types for this matchup."""
        recommendations = []
        
        # Favor plays where offense has advantage
        if run_advantage in [MatchupAdvantage.MAJOR_ADVANTAGE, MatchupAdvantage.MINOR_ADVANTAGE]:
            recommendations.extend([PlayType.RUN_INSIDE, PlayType.RUN_OUTSIDE])
        
        if pass_advantage in [MatchupAdvantage.MAJOR_ADVANTAGE, MatchupAdvantage.MINOR_ADVANTAGE]:
        ...
        for play_type in offense.optimal_play_types:
            if play_type not in recommendations and play_type not in defense.counters_play_types:
        ...
        if run_advantage != MatchupAdvantage.MAJOR_ADVANTAGE:
            recommendations = [p for p in recommendations if p not in defense.counters_play_types 
    ...
    def get_formation_summary(self, formation_name: str, is_offense: bool = True) -> Optional[Dict]:
        ...
        if is_offense:
            ...
            if formation:
                return {
                    ...
                    "optimal_plays": [play.value for play in formation.optimal_play_types]
                }
        else:
