...
class PlayLoader:
    ...
    def _create_play_from_data(self, data: Dict[str, Any]) -> FootballPlay:
        ...
        defensive_reactions = self._parse_defensive_reactions(
            data.get("defensive_reactions", [])
        )
    ...
    def _parse_defensive_reactions(
        self, reactions_data: List[Dict]
    ) -> List[DefensiveMotionReaction]:
    ...
    def validate_play_against_formation(self, play: FootballPlay) -> List[str]:
        ...
        try:
            ...
            play_players = set(
                assignment.player_position for assignment in play.assignments
            )

            missing_players = play_players - formation_players
            if missing_players:
                violations.append(
                    f"Play references players not in formation: {missing_players}"
                )
            ...
            violations.append(
                f"Cannot validate against formation {play.base_formation}: {e}"
            )
