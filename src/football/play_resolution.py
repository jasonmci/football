...
from football.play_analyzer import (
    PlayAnalyzer,
    PlayAnalysis,
)
...
class ResolutionConfig:
    ...
    base_dice: Dict[PlayType, str] = field(
    ...
    thresholds: Dict[PlayOutcome, int] = field(
        default_factory=lambda: {
    ...
    formation_bonuses: Dict[int, int] = field(
        default_factory=lambda: {
    ...
    down_distance_modifiers: Dict[str, int] = field(
        default_factory=lambda: {
            "1st_and_10": 0,
    ...
    yardage_ranges: Dict[PlayOutcome, Tuple[int, int]] = field(
        default_factory=lambda: {
...
class PlayResolutionEngine:
    ...
    def __init__(
        self, config: Optional[ResolutionConfig] = None, seed: Optional[int] = None
    ):
    ...
    def resolve_play(
        ...
        try:
            matchup = self.matchup_analyzer.analyze_matchup(
                offensive_play.base_formation, defensive_play.base_formation
            )
        except ValueError:
        ...
        play_analysis = self.play_analyzer.analyze_play_matchup(
        ...
        modifiers = self._calculate_modifiers(
            offensive_play, defensive_play, matchup, situation, play_analysis
        )
        ...
        advantage, disadvantage = self._calculate_dice_advantage(
            modifiers, matchup, play_type, play_analysis
        )
    ...
    def _calculate_modifiers(
        ...
        modifiers["situation"] = self.config.down_distance_modifiers.get(
            situation_key, 0
        )
