...
class PlayExecutionResult:
    ...
    def description(self) -> str:
        ...
        if self.missed_tackles > 0:
            desc += (
                f" ({self.missed_tackles} missed tackle"
                f"{'s' if self.missed_tackles > 1 else ''})"
            )
...
class EnhancedResolutionEngine:
    ...
    def resolve_pass_play(
        ...
        missed_tackles = self._calculate_missed_tackles(receiver, defender, yac)
    ...
    def resolve_run_play(
        ...
        missed_tackles = self._calculate_run_missed_tackles(runner, defenders, yac)
    ...
    def _calculate_missed_tackles(
        self, ball_carrier: PlayerProfile, defender: PlayerProfile, yards_after: int
    ) -> int:
    ...
    def _calculate_run_after_contact(
        self, runner: PlayerProfile, defenders: List[PlayerProfile], base_yac: int
    ) -> int:
        ...
        avg_tackle = sum(d.get_skill(SkillCategory.TACKLE) for d in defenders) / len(
            defenders
        )
    ...
    def _calculate_run_missed_tackles(
        self, runner: PlayerProfile, defenders: List[PlayerProfile], yac: int
    ) -> int:
    ...
    def _calculate_fumble_risk(
        self, runner: PlayerProfile, defenders: List[PlayerProfile], total_yards: int
    ) -> int:
    ...
    def _calculate_interception_risk(
        ...
        base_risk += self._interception_risk_qb_awareness(qb)
        base_risk += self._interception_risk_qb_traits(qb)
        base_risk += self._interception_risk_defender_skills(defender)
        base_risk += self._interception_risk_pressure(situation)
        base_risk += self._interception_risk_route_difficulty(base_result)

        return max(0, min(25, base_risk))  # Cap at 25% for incomplete passes

    def _interception_risk_qb_awareness(self, qb: PlayerProfile) -> int:
        qb_awareness = qb.get_skill(SkillCategory.AWARENESS)
        if qb_awareness >= 90:
            return -3
        elif qb_awareness >= 80:
            return -1
        elif qb_awareness < 70:
            return 2
        return 0

    def _interception_risk_qb_traits(self, qb: PlayerProfile) -> int:
    ...
    def _interception_risk_defender_skills(self, defender: PlayerProfile) -> int:
        def_coverage = defender.get_skill(SkillCategory.COVERAGE)
        ...
        if avg_def_skill >= 85:
            return 2
        elif avg_def_skill >= 75:
            return 1
        elif avg_def_skill < 65:
            return -2
        return 0

    def _interception_risk_pressure(self, situation: Dict[str, Any]) -> int:
        if situation.get("pass_rush_pressure", False):
            return 3
        return 0

    def _interception_risk_route_difficulty(self, base_result: Any) -> int:
        if base_result.yards_gained > 15:
            return 2
        elif base_result.yards_gained <= 5:
            return -1
        return 0
...
def create_sample_players():
    ...
    elite_qb = PlayerProfile(
        ...
        position="QB",
        overall_rating=92,
        skills={SkillCategory.AWARENESS: 95, SkillCategory.HANDS: 90},  # For handoffs
    )
    ...
    average_cb = PlayerProfile(
        ...
        position="CB",
        overall_rating=75,
        skills={SkillCategory.COVERAGE: 76, SkillCategory.TACKLE: 72},
    )
...
if __name__ == "__main__":
    ...
    # Test run play
    print("\n🏃 Run Play Result:")
    run_result = engine.resolve_run_play(rb, [], [cb], base_result, {})
