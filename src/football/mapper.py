...
from .schemas import PlayLogicalFramesDTO, PlayFramesDTO, Team, RoleSpec, PlayerFrame, FormationSpecDTO
...
def _base_xy(team: Team, pos: str, lane: str, depth: str, maps, cols: int, rows: int) -> tuple[int,int]:
    ...
    y = ymap.get(depth, (DEFAULT_DEPTH_Y_OFFENSE if team == "offense" else DEFAULT_DEPTH_Y_DEFENSE).get(depth, 10))
    x = max(0, min(cols-1, x)); y = max(0, min(rows-1, y))
    return x, y

def _apply_placement(x: int, y: int, role_name: str, formation: FormationSpecDTO, cols: int, rows: int) -> tuple[int,int]:
    p = formation.placement.get(role_name)
    if not p: return x, y
    if p.x is not None: x = p.x
    if p.y is not None: y = p.y
    x += p.dx; y += p.dy
...
def render_frames(
    ...
    cols: int = 20, rows: int = 20, force_reload_config: bool = False
) -> PlayFramesDTO:
