...
ROUTE_SET = {
    "slant","hitch","out","in","drag","post","corner","go","screen_bubble"
}

# Offensive actions:
# - OL actions expanded to include 'combo' and 'block_down' (useful for TEs inline too)
OL_ACTIONS = {
    "pull","trap","double","reach","down","climb","pass_protect","combo","block_down",
...
SKILL_ACTIONS = {
    "route","run","screen","chip_release","pass_protect",
    "stalk","crack","clear","swing","lead","seal","block_down","backside_cutoff"
}
...
# Front / rush actions
FRONT_ACTIONS = {
    "blitz","stunt","edge_set","spy","contain","rush_3"
}
...
class PlayLoadError(ValueError): pass
    ...
    for p in data.get("plays", []):
        ...
        if motion:
            ...
            for wp in motion["path"]:
                lane = wp.get("lane"); depth = wp.get("depth")
        ...
        for pre in p.get("pre_snap", []):
            ...
            if t == "shift_front":
                if pre.get("direction") not in {"left","right","pinch","wide"}:
                    raise PlayLoadError(f"{key}: front shift '{pre.get('direction')}' invalid")
            elif t == "safety_roll":
                if pre.get("to_depth") not in {"box","deep"}:
                    raise PlayLoadError(f"{key}: safety_roll to_depth '{pre.get('to_depth')}' invalid")
            # pre-snap: show_blitz lane validation
            elif t == "show_blitz":
                if pre.get("lane") and pre["lane"] not in LANES_EXT:   # was LANES
                    raise PlayLoadError(f"{key}: show_blitz lane '{pre['lane']}' invalid")
            else:
            ...
            if act in FRONT_ACTIONS:
                ...
                if act == "stunt":
                    if a.get("path") and a["path"] not in STUNT_PATHS:
                        raise PlayLoadError(f"{key}: stunt path '{a.get('path')}' invalid")
