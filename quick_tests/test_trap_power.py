...
def test_new_run_concepts():
    ...
    for play_name in trap_plays:
        test_run_concept(
            offensive_plays[play_name], basic_def, analyzer, engine, "TRAP"
        )

    for play_name in power_plays:
        test_run_concept(
            offensive_plays[play_name], basic_def, analyzer, engine, "POWER"
        )
...
def test_run_concept(off_play, def_play, analyzer, engine, concept_type):
    ...
    for assign in off_play.assignments:
        ...
        if highlights:
            print(
                f"  {assign.player_position}: {assign.assignment_type.value} {' '.join(highlights)}"
            )
    ...
    result = engine.resolve_play(
        off_play, def_play, {"down": 2, "distance": 5, "field_position": 35}
    )

    print(f"\n🎲 Resolution: {result.outcome.value} for {result.yards_gained:+d} yards")
    print(
        f"   Total: {result.final_total} (dice: {result.dice_roll} + mods: {result.total_modifier})"
    )
