    ...
    # Quick Slant vs defenses  
        ...
        if defense_name == "run_blitz":
            ...
            return PlayAnalysis(
                ...
                scheme_analysis={"type": "coverage_vs_quick", "advantage": "slight_defense"},
                confidence=0.70
            )
            
    # Deep Post vs defenses
    ...
    for scenario_name, situation in scenarios:
        ...
        for off_name, def_name, matchup_desc in key_matchups:
            ...
            if results:
                ...
                print(f"    📊 Results: {avg_yards:.1f} avg yds ({worst_yards} to {best_yards})")
                print(f"    ✅ Success Rate: {success_rate:.0f}% | Net Impact: {analysis.net_impact:+d}")
                ...
                if analysis.disadvantages:
                    disadvantages_str = ", ".join([f.factor_type for f in analysis.disadvantages[:2]])
                    print(f"    ⚠️  Disadvantages: {disadvantages_str}")
                
                # Strategic assessment
                if analysis.net_impact >= 2:
                    print(f"    🎯 Tactical Assessment: Strong offensive advantage")
                elif analysis.net_impact <= -2:
                    print(f"    🛡️  Tactical Assessment: Strong defensive advantage")
                else:
                    print(f"    ⚖️  Tactical Assessment: Balanced matchup")
