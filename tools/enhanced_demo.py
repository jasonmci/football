...
class EnhancedPlaySelectorDemo:
    ...
    def _calculate_modifiers(self, offense_data, defense_data, play_category) -> List[int]:
        ...
        if play_category == 'rpo':
            ...
            if 'blitz' in def_advantages:
                modifiers.append(-mod_config['perfect_counter'])  # Draw vs blitz is good
    ...
    def run_demo_simulation(self, offensive_play, defensive_play, description):
        ...
        print(f"Outcome Table:")
        for roll, data in table.items():
            decision_str = f" [{data.get('decision', '').upper()}]" if data.get('decision') and play_category == 'rpo' else ""
            print(f"  {roll}: {data['kind']} ({data['y'][0]} to {data['y'][1]} yards){decision_str}")
        ...
        for i in range(10):
            ...
            print(f"Sim {i+1:2d}: {yards:+3d} yards - {outcome:>12}{rpo_str} | Dice: {roll} → {eff_roll} {mod_str}")
        ...
        print("-" * 75)
        print(f"SUMMARY:")
        print(f"  Average: {avg_yards:+5.1f} yards  |  Range: {min_yards:+3d} to {max_yards:+3d} yards")
        print(f"  Outcomes: {', '.join(f'{outcome}({count})' for outcome, count in sorted(outcomes.items()))}")
        
        # Show RPO decision breakdown if applicable
        if rpo_decisions:
            print(f"  RPO Decisions: {', '.join(f'{decision}({count})' for decision, count in sorted(rpo_decisions.items()))}")
