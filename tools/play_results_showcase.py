...
from football2.football.play_resolution import PlayResolutionEngine, create_realistic_config
...
def create_realistic_analysis(off_name: str, def_name: str, situation: dict) -> PlayAnalysis:
    """Create realistic tactical analysis based on game situation."""
    
    # Trap Right analysis
    if off_name == "trap_right":
        if def_name == "base_43":
            return PlayAnalysis(
                advantages=[
                    PlayMatchupFactor("TRAP_CONCEPT", +1, "LG pulls, RG invites DT penetration"),
                    PlayMatchupFactor("MISDIRECTION", +1, "Backfield fake sells power right")
                ],
                disadvantages=[],
                net_impact=2,
                key_matchups=["LG vs LOLB", "RG vs NT"],
                scheme_analysis={"concept": "gap_scheme_misdirection", "blocking": "invite_and_trap"},
                confidence=0.80
            )
            ...
            return PlayAnalysis(
                ...
                scheme_analysis={"concept": "trap_vs_blitz", "advantage": "scheme_beats_aggression"},
                confidence=0.75
            )
    
    # Power Right analysis
    elif off_name == "power_right":
        if def_name == "base_43":
            return PlayAnalysis(
                advantages=[
                ...
                scheme_analysis={"concept": "gap_control", "blocking": "displacement_physics"},
                confidence=0.85
            )
        elif def_name == "run_blitz":
            return PlayAnalysis(
                advantages=[
                    PlayMatchupFactor("LEAD_BLOCKER", +1, "FB picks up blitzer")
                ],
                disadvantages=[
    ...
    elif off_name == "outside_zone":
        if def_name == "nickel_coverage":
            return PlayAnalysis(
                advantages=[
                    PlayMatchupFactor("LIGHT_BOX", +2, "Only 6 defenders in box"),
                    PlayMatchupFactor("STRETCH_CONCEPT", +1, "Horizontal stretch creates gaps")
                ],
                disadvantages=[],
                net_impact=3,
                key_matchups=["OL vs DL", "RB vs Safety"],
                scheme_analysis={"concept": "zone_stretch", "advantage": "numbers_game"},
                confidence=0.85
            )
        elif def_name == "run_blitz":
            return PlayAnalysis(
                advantages=[],
                disadvantages=[
                    PlayMatchupFactor("BLITZ_DISRUPTION", -2, "MLB blitz disrupts zone timing"),
                    PlayMatchupFactor("HARD_PURSUIT", -1, "Defense flows fast outside")
                ],
                net_impact=-3,
                key_matchups=["OL vs DL + Blitzer", "RB vs fast pursuit"],
                scheme_analysis={"concept": "zone_vs_blitz", "disadvantage": "timing_disrupted"},
                confidence=0.80
            )
    
    # Quick Slant analysis
    elif off_name == "quick_slant":
        if def_name == "run_blitz":
            return PlayAnalysis(
                advantages=[
                    PlayMatchupFactor("HOT_ROUTE", +2, "Quick release beats blitz timing"),
                    PlayMatchupFactor("VACATED_COVERAGE", +1, "Blitzer leaves hole in coverage")
                ],
                disadvantages=[],
                net_impact=3,
                key_matchups=["WR vs CB", "QB vs pass rush"],
                scheme_analysis={"concept": "timing_vs_pressure", "advantage": "chess_match_won"},
                confidence=0.90
            )
            ...
            return PlayAnalysis(
                ...
                scheme_analysis={"concept": "quick_vs_coverage", "result": "defensive_advantage"},
                confidence=0.70
            )
    
    # Play Action analysis
    elif off_name == "play_action":
        if def_name == "base_43":
            return PlayAnalysis(
                advantages=[
                    PlayMatchupFactor("PLAY_ACTION", +2, "LBs bite on run fake"),
                    PlayMatchupFactor("DEEP_CONCEPT", +1, "Route combinations stress coverage")
                ],
                disadvantages=[
                ...
                scheme_analysis={"concept": "misdirection_pass", "advantage": "sells_run_well"},
                confidence=0.75
            )
...
def generate_play_narrative(offense: FootballPlay, defense: FootballPlay, result, analysis: PlayAnalysis, situation: dict) -> str:
    """Generate detailed play-by-play narrative."""
    
    down_desc = f"{situation['down']}{['st', 'nd', 'rd', 'th'][min(situation['down']-1, 3)]} and {situation['distance']}"
    ...
    if offense.play_type == "run":
        if "trap" in offense.name:
            narrative += f"   QB hands off to RB on apparent power right...\n"
            narrative += f"   But LG pulls behind the line while RG invites penetration!\n"
            if result.yards_gained > 3:
                narrative += f"   RB cuts behind the trap block and finds daylight!\n"
            elif result.yards_gained > 0:
                narrative += f"   RB follows the pulling guard for a solid gain.\n"
            else:
                narrative += f"   Defense doesn't bite on the fake - minimal gain.\n"
        
        elif "power" in offense.name:
            narrative += f"   FB leads through the B-gap as RG and RT double-team!\n"
            if result.yards_gained > 3:
                narrative += f"   The double team creates movement, RB bursts through!\n"
            elif result.yards_gained > 0:
                narrative += f"   Physical blocking creates enough space for a decent gain.\n"
            else:
                narrative += f"   Defense holds strong at the point of attack.\n"
        
        elif "zone" in offense.name:
            narrative += f"   OL zone blocks left as RB reads the defense...\n"
            if result.yards_gained > 3:
                narrative += f"   A hole opens up and RB cuts through for good yardage!\n"
            elif result.yards_gained > 0:
                narrative += f"   RB finds the crease and gets what's available.\n"
            else:
                narrative += f"   Defense strings the play out to the sideline.\n"
    
    else:  # Pass play
        if "slant" in offense.name:
            narrative += f"   Quick 3-step drop, slant route at the sticks...\n"
            if result.yards_gained > 5:
                narrative += f"   WR catches in stride and picks up extra yards!\n"
            elif result.yards_gained > 0:
                narrative += f"   Quick completion, WR secured the catch.\n"
            else:
                narrative += f"   Tight coverage, no room for the receiver.\n"
        
        elif "action" in offense.name:
            narrative += f"   Play action fake freezes the linebackers...\n"
            if result.yards_gained > 8:
                narrative += f"   WR breaks free downfield - big completion!\n"
            elif result.yards_gained > 0:
                narrative += f"   QB finds his target for a nice gain.\n"
            else:
                narrative += f"   Pass rush gets home before routes develop.\n"
    ...
    if result.yards_gained > 0:
        ...
        narrative += f"Loss of {abs(result.yards_gained)} yards ({result.outcome.name})\n"
...
def run_play_results_showcase():
    ...
    game_scenarios = [
        {
            ...
            "plays": [
                ("trap_right", "base_43", {"down": 1, "distance": 10, "field_position": 35}),
                ("power_right", "base_43", {"down": 2, "distance": 7, "field_position": 37}),
                ("outside_zone", "run_blitz", {"down": 3, "distance": 4, "field_position": 40})
            ]
        },
        {
            "description": "Red Zone Pressure - Short Yardage",
            "plays": [
                ("power_right", "base_43", {"down": 1, "distance": 10, "field_position": 18}),
                ("trap_right", "run_blitz", {"down": 2, "distance": 7, "field_position": 21}),
                ("quick_slant", "nickel_coverage", {"down": 3, "distance": 4, "field_position": 24})
            ]
        },
        {
            "description": "Two-Minute Drill - Passing Game",
            "plays": [
                ("quick_slant", "run_blitz", {"down": 1, "distance": 10, "field_position": 45}),
                ("play_action", "base_43", {"down": 2, "distance": 6, "field_position": 49}),
                ("outside_zone", "nickel_coverage", {"down": 3, "distance": 3, "field_position": 52})
            ]
        }
    ]
    ...
    for scenario in game_scenarios:
        ...
        for play_data in scenario['plays']:
            ...
            narrative = generate_play_narrative(offense, defense, result, analysis, situation)
            print(narrative)
