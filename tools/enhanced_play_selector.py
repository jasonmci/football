...
class EnhancedPlaySelector:
    ...
    def _determine_play_category(self, offense_data) -> str:
        ...
        # Check running plays
            ...
            return 'outside_run'
        elif any(word in text for word in ['power', 'dive', 'inside', 'zone', 'trap', 'counter']):
            return 'inside_run'
        # Check passing plays
        elif any(word in text for word in ['screen', 'bubble']):
            return 'screen_pass'
        elif any(word in text for word in ['te_', 'tight_end']) or name.startswith('te_'):
            return 'te_route'
        elif any(word in text for word in ['deep', 'vert', 'vertical', 'go', 'four_verts']):
            return 'deep_pass'
        elif any(word in text for word in ['slant', 'quick', 'short', 'hitch', 'empty']):
            return 'short_pass'
    ...
    def _calculate_modifiers(self, offense_data, defense_data, play_category) -> List[int]:
        ...
        if play_category == 'rpo':
            ...
            if 'power_running' in off_advantages and 'gap_control' in def_advantages:
                ...
                pass  # Even matchup
            elif 'misdirection' in off_advantages and 'disciplined_pursuit' not in def_advantages:
                has_offensive_advantage = True
            # SWEEP vs SWEEP DEFENSE - specific counter
            elif play_category == 'outside_run' and 'sweep_stop' in def_advantages:
                has_defensive_advantage = True
                modifiers.append(mod_config['perfect_counter'])  # Major penalty for sweep vs sweep defense
            elif play_category == 'outside_run' and 'eight_man_box' in def_advantages:
                has_defensive_advantage = True
                modifiers.append(mod_config['tactical_disadvantage'])  # Extra penalty for 8-man box
                
        elif play_category in ['short_pass', 'deep_pass', 'te_route', 'screen_pass']:
            # Passing play matchups
            ...
            elif 'pattern_matching' in def_advantages and 'bunch_formation' in off_advantages:
                has_defensive_advantage = True
        ...
        if ('blitz' in def_advantages and play_category == 'draw_play'):
            modifiers.append(-mod_config['perfect_counter'])  # Draw vs blitz is good for offense
        elif ('run_commit' in def_advantages and play_category in ['inside_run', 'outside_run']):
            modifiers.append(mod_config['perfect_counter'])  # Run commit vs run is good for defense
        
        return modifiers
    
    def _check_turnover(self, effective_roll: int, play_category: str) -> Optional[Tuple[str, int]]:
        """Check if play results in turnover"""
        turnover_config = self.config['turnovers']
        
        if play_category in ['short_pass', 'deep_pass', 'te_route', 'screen_pass', 'rpo']:
    ...
    def _resolve_play_outcome(self, offense_data, defense_data) -> Dict[str, Any]:
        ...
        rpo_decision = None
        if play_category == 'rpo':
            rpo_decision = outcome_data.get('decision', 'run')  # Default to run if not specified
    ...
    def simulate_play_matchup(self, offensive_play, defensive_play):
        ...
        try:
            ...
            print(f"\n{'='*70}")
            print(f"SIMULATION: {offensive_play.replace('_', ' ').title()} vs {defensive_play.replace('_', ' ').title()}")
            print(f"{'='*70}")
            ...
            for i in range(10):
                ...
                category = result['play_category']
                rpo_decision = result.get('rpo_decision')
                
                mod_str = f"(+{sum(mods)})" if sum(mods) > 0 else f"({sum(mods)})" if sum(mods) < 0 else ""
                
                # Add RPO decision to display
                rpo_str = f" [{rpo_decision.upper()}]" if rpo_decision else ""
                
                print(f"Sim {i+1:2d}: {yards:+3d} yards - {outcome:>12}{rpo_str} | Roll: {roll} → {eff_roll} {mod_str}")
            ...
            print("-" * 70)
            print(f"SUMMARY ({len(results)} simulations using {self.config['dice']['core']}):")
