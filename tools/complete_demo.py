#!/usr/bin/env python3
"""
Complete Demo of Interactive Play Selector - Shows all example simulations
"""
...
class PlaySelectorDemo:
    ...
    def _simulate_single_play(self, offense_data, defense_data):
        ...
        base_yards, outcomes, weights = self._get_base_simulation_params(play_type)
        base_yards, weights = self._apply_tactical_adjustments(
            play_type, base_yards, weights, off_advantages, def_advantages
        )

        outcome = random.choices(outcomes, weights=weights)[0]
        base_yards = self._adjust_yards_for_outcome(base_yards, outcome)

        return {"yards": base_yards, "outcome": outcome}

    def _get_base_simulation_params(self, play_type):
        if play_type == "run":
    ...
    def _apply_tactical_adjustments(
        self, play_type, base_yards, weights, off_advantages, def_advantages
    ):
        if play_type == "run":
            if (
                "goal_line_power" in off_advantages
                and "goal_line_stop" in def_advantages
            ):
        ...
        return base_yards, weights

    def _adjust_yards_for_outcome(self, base_yards, outcome):
        if outcome in ["stuffed", "failed", "sack"]:
            return min(base_yards, random.randint(-3, 1))
        elif outcome in ["big_gain", "big_play"]:
            return max(base_yards, random.randint(8, 25))
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("FOOTBALL PLAY SIMULATOR - Complete Demo")
    print("=" * 70)
    ...
    # Run example simulations showing different scenarios
    examples = [
        ("power_right", "bear46_run_commit", "Power run vs run-stopping defense"),
        ("te_seam_concept", "cover2_robber_te", "TE route vs TE-specific coverage"),
        ...
        ("sweep_right", "eight_man_box_sweep_stop", "Outside run vs sweep defense"),
        ("empty_slants", "nickel_doublea_cover2", "Quick passing vs blitz"),
    ]
    ...
    print("\n" + "=" * 70)
    print("DEMO COMPLETE!")
    print("To use interactively, run: python tools/interactive_play_selector.py")
    print("=" * 70)
