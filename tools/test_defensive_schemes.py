...
def create_realistic_players():
    ...
    return {
        # Elite tier
        "mahomes": PlayerProfile("Patrick Mahomes", "QB", 96, {
            SkillCategory.AWARENESS: 97, SkillCategory.HANDS: 92
        }),
        "hill": PlayerProfile("Tyreek Hill", "WR", 93, {
            SkillCategory.HANDS: 88, SkillCategory.ROUTE_RUNNING: 90, 
            SkillCategory.SPEED: 99, SkillCategory.AGILITY: 95
        }),
        "henry": PlayerProfile("Derrick Henry", "RB", 91, {
            SkillCategory.STRENGTH: 95, SkillCategory.SPEED: 88, SkillCategory.AGILITY: 78
        }),
        "ramsey": PlayerProfile("Jalen Ramsey", "CB", 94, {
            SkillCategory.COVERAGE: 96, SkillCategory.TACKLE: 87
        }),
        "watt": PlayerProfile("T.J. Watt", "LB", 95, {
            SkillCategory.PASS_RUSH: 97, SkillCategory.TACKLE: 88
        }),
        
        # Good tier
        "cousins": PlayerProfile("Kirk Cousins", "QB", 83, {
            SkillCategory.AWARENESS: 85, SkillCategory.HANDS: 82
        }),
        "robinson": PlayerProfile("Allen Robinson", "WR", 82, {
            SkillCategory.HANDS: 87, SkillCategory.ROUTE_RUNNING: 85,
            SkillCategory.SPEED: 78, SkillCategory.AGILITY: 80
        }),
        "cook": PlayerProfile("Dalvin Cook", "RB", 87, {
            SkillCategory.STRENGTH: 79, SkillCategory.SPEED: 91, SkillCategory.AGILITY: 93
        }),
        "alexander": PlayerProfile("Jaire Alexander", "CB", 88, {
            SkillCategory.COVERAGE: 90, SkillCategory.TACKLE: 82
        }),
        
        # Average tier
        "backup_qb": PlayerProfile("Backup QB", "QB", 74, {
            SkillCategory.AWARENESS: 75, SkillCategory.HANDS: 74
        }),
        "depth_wr": PlayerProfile("Depth WR", "WR", 71, {
            SkillCategory.HANDS: 73, SkillCategory.ROUTE_RUNNING: 69,
            SkillCategory.SPEED: 76, SkillCategory.AGILITY: 72
        }),
        "avg_cb": PlayerProfile("Average CB", "CB", 75, {
    ...
    if off_play["play"].play_type == "pass":
        ...
        if route_depth <= 6:  # Quick routes
            if "blitz" in def_scheme["play"].name:
                advantages.append(PlayMatchupFactor("QUICK_RELEASE", +2, "Ball out before blitz arrives"))
            elif "cover_3" in def_scheme["play"].name:
                advantages.append(PlayMatchupFactor("UNDERNEATH_SOFT", +1, "Soft underneath coverage"))
            elif "prevent" in def_scheme["play"].name:
                advantages.append(PlayMatchupFactor("PREVENT_WEAKNESS", +2, "Prevent gives up underneath"))
                
        # Deep passes vs coverages
        elif route_depth >= 15:  # Deep routes
            if "cover_2" in def_scheme["play"].name:
                disadvantages.append(PlayMatchupFactor("DEEP_COVERAGE", -1, "Cover-2 has deep help"))
            elif "prevent" in def_scheme["play"].name:
                disadvantages.append(PlayMatchupFactor("DEEP_HELP", -3, "Prevent designed to stop deep"))
            elif "blitz" in def_scheme["play"].name:
                disadvantages.append(PlayMatchupFactor("PRESSURE_VS_DEEP", -2, "Blitz disrupts deep timing"))
    
    else:  # Run plays
        if "power" in off_play["play"].name:
            if "goal_line" in def_scheme["play"].name:
                disadvantages.append(PlayMatchupFactor("HEAVY_BOX", -2, "6-man front stops power"))
            elif "nickel" in def_scheme["play"].name:
                advantages.append(PlayMatchupFactor("LIGHT_BOX", +2, "Nickel = fewer run defenders"))
                
        elif "zone" in off_play["play"].name:
            if "blitz" in def_scheme["play"].name:
                disadvantages.append(PlayMatchupFactor("BLITZ_DISRUPTION", -2, "Blitz disrupts zone timing"))
            elif "prevent" in def_scheme["play"].name:
                advantages.append(PlayMatchupFactor("PREVENT_RUN", +3, "Prevent can't stop runs"))
...
def run_complete_analysis():
    ...
    base_engine = PlayResolutionEngine(create_realistic_config())
    ...
    for scenario in test_scenarios:
        ...
        if len(scenario['player_combo']) > 1:
            print(f"   👥 {players[scenario['player_combo'][0]].name} ➜ {players[scenario['player_combo'][1]].name}")
        else:
        ...
        for def_name in scenario["defenses"]:
            ...
            base_result = SimpleNamespace(
                ...
                yards_gained=max(1, off_play.get("route_depth", 4) + analysis.net_impact),
                dice_roll=11,
            ...
            if off_play["play"].play_type == "pass":
                ...
                print(f"   📊 Result: {completion_rate:.0f}% completion | {avg_yards:.1f} avg yds | {avg_yac:.1f} YAC")
            else:
            ...
            if analysis.net_impact >= 2:
                print(f"   🎯 Assessment: **Strong offensive advantage**")
            elif analysis.net_impact <= -2:
                print(f"   🛡️  Assessment: **Strong defensive advantage**") 
            else:
                print(f"   ⚖️  Assessment: **Balanced matchup**")
