...
def create_test_players():
    ...
    elite_lg = PlayerProfile(
        ...
        overall_rating=86,
        skills={
            SkillCategory.STRENGTH: 89,
            SkillCategory.RUN_BLOCKING: 90
        },
        traits=[]
    )
...
def main():
    ...
    for base_yards in base_yards_tests:
        ...
        for i in range(5):
            ...
            print(f"  Run {i+1}: {result.outcome:12s} | {result.yards_gained:2d} yards | YAC: {getattr(result, 'yards_after_contact', 0)}")
