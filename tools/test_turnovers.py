    ...
    for scenario in scenarios:
        ...
        for _ in range(100):
            ...
            situation = {
                ...
                "defenders_nearby": [players['strong_lb']] if scenario['yards'] <= 5 else []
            }
            
            result = engine.resolve_pass_play(
                scenario['qb'], scenario['wr'], scenario['def'],
                base_result, situation
            )
        ...
        int_rate_of_incompletions = (interceptions / (incompletions + interceptions)) * 100 if (incompletions + interceptions) > 0 else 0
        
        print(f"   Results over 100 attempts:")
    ...
    for scenario in scenarios:
        ...
        for _ in range(100):
            ...
            result = engine.resolve_run_play(
                scenario['rb'], [], scenario['defenders'],
                base_result, situation
            )
...
def test_realistic_game_situation():
    ...
    situation = {
        ...
        "defenders_nearby": [players['strong_lb']]
    }
    
    result = engine.resolve_pass_play(
        players['mistake_qb'], players['average_wr'], players['ball_hawk'],
        base_result, situation
    )
    ...
    if result.outcome not in ["INTERCEPTION"]:
        ...
        run_result = engine.resolve_run_play(
            players['secure_rb'], [], [players['strong_lb']],
            run_result_base, {"contact_level": "normal"}
        )
