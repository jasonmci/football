...
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            with pytest.raises(ValueError, match="rb\\(3\\) \\+ te\\(3\\) exceeds 5 skill players"):
...
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            with pytest.raises(ValueError, match="illegal alignment for TE: middle/wide"):
                load_off_formations(f.name)
...
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            with pytest.raises(ValueError, match="offensive formation totals 6, expected 11"):
                load_off_formations(f.name)
...
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            with pytest.raises(ValueError, match="illegal defensive depth 'invalid_depth'"):
...
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            with pytest.raises(ValueError, match="defensive formation totals 5, expected 11"):
                load_def_formations(f.name)
...
class TestAllowedCombos:
    ...
    def test_allowed_combos_structure(self):
        ...
        # Test TE flexibility
        expected_te = {("left", "line"), ("right", "line"), ("left", "wide"), ("right", "wide")}
